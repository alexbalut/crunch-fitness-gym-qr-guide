...
Members never log in. `/` is the in-product gym home for **Crunch Fitness Lachine**.
...
3. Friendly URLs: `/m/crunch-fitness/lat-pulldown`.
...
Keyed by gym slug (`crunch-fitness`). No member account.

| Key | Contents |
|-----|----------|
| `crunch-fitness-workout:v1:<slug>` | In-progress session JSON |
| `crunch-fitness-workout-history:v1:<slug>` | Array of saved workouts (newest first) |
