...
Seed gym: `crunch-fitness`, `primaryColor: #E51937`, `secondaryColor: #000000`.

## Multi-tenant gym

Admin session carries `gymId`. APIs scope mutations to that gym. Member home loads slug `crunch-fitness` (fallback: first gym).
...
| Workout + history | localStorage keys `crunch-fitness-workout:v1:<slug>` and `crunch-fitness-workout-history:v1:<slug>` |
